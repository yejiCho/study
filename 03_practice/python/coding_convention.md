...
2. 블럭 주석, 인라인 주석
* 블록 주석: 블록 안에 쓰이는 주석 
반복문 혹은 조건문이 어떤 기능을 하는지 설명
작성 방식: 블록 시작 첫 줄에 코드와 같은 위치에 들여쓰기
# 입력 후 한칸 띄어쓰기, 내용작성
조금 복잡하거나 설명이 필요한 부분에 작성하면 좋을듯
ex)
sum = 0
for i in range(1, 11):
    # 1~10까지의 곱을 구한다
    sum *= 10
블록 안에 코드를 구분할 때도 사용함(조건 등이 길때)
ex)
if (name == 'name' or
    age == 'age' or
    job == 'job'):
...
* 인라인 주석: 코드 옆에 쓰이는 주석
인라인 주석은 적당히 사용할 것
사실 별로 필요 없지만 특별한 곳에 명확하게 사용한다.
작성 방식: 코드와 최소 2개의 스페이스로 구분(tab 1개 정도면 적당할듯)
# 입력 후 한칸 띄어쓰기
ex)
x = x + 1   # x 증가
위와 같은 인라인 주석은 안쓰는게 좋음
all_contact = dict()    # 전체 연락처를 저장하는 딕셔너리
FILE_NAME = "contact.pickle"    # 연락처를 저장할 파일 경로
위와 같이 특별한 부분에 써주면 유용할 듯
```
```python
3. 닥스트링(Documentation String, DocString)
"""주석""" 구성
모든 public 모듈, 함수, 클래스, 메소드에 대해 닥스트링 작성
함수이름.__doc__ 를 print하면 해당 내용이 출력됨
...
```
4. 주석 마무리
함수/변수 주석은 달아주면 좋을 거 같고
모듈, 함수, 클래스, 메소드에는 닥스트링을 달아준다.
반복문, 조건문 같은 블록 안에는 필요에 따라 블록 주석을 달아준다.
인라인 주석은 최대한 안쓰더라도 특별한 코드 부분에는 달아줄 수 있다.
```
