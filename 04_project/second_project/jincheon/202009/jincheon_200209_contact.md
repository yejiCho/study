...
def get_menu():
    ...
    5. 종료""")
    try:
        menu_num = get_integer(
            input("메뉴 번호 입력>> ").strip(), 1, 6)
...
def input_one_contact() -> dict:
    ...
    while 1:
        try:
            ph_num = get_phone_number(
                input("전화번호[ex) 01012345678]: ").strip())
            break
    ...
    while 1:
        try:
            cfc = get_classification(
                input("구분[ex) 1.가족, 2.친구, 3.회사, 4.기타]: ").strip())
            break
    ...
    def edit_contact(self, mode: str):
        ...
        if match_keys:
            ...
            while 1:
                ...
                try:
                    num = get_integer(
                        input(f"{mode}할 번호 입력(0: 종료)>> ").strip(),
                        0, len(match_keys)+1)
                except InputNumberError as e:
                    print(e)
                    continue
