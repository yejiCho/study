...
class File:     ## 파일을 직접 로드,저장하는 클래스
    ...
    def picWrite(self,fileName):
        self.fileName=fileName
        with open(fileName,"wb") as contact:
            data = total
            pickle.dump(data, contact)


class FileManager:      ## 파일을 직접 관리하는 클래스
    def add(self, aOM):
...
inputFile = "contact.txt"
total = File.picLoad(inputFile)
mark = True
while(mark):
    mainMenu()
    num = input()
    if num == '1':
        ...
        FileManager.delOrMod("수정")

    elif num == '4':
        FileManager.delOrMod("삭제")

    elif num == '5':
        File.picWrite(inputFile)
        mark = False
